...
import { useThreeColumnGallery } from '@/hooks/gallerrycards';
...
export function ThreeColumnGallery({
  ...
  return (
    <>
      <section className={`w-full bg-white py-16 md:py-20 ${className}`}>
            ...
            {cards.map((card) => (
              <article key={card.id} className='group w-full overflow-hidden'>
                <button
                  ...
                  aria-label={`View image ${card.alt}`}
                  className='
                    relative
                    block
                    w-full
